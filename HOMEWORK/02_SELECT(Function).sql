--춘 대학교 워크북 과제 SQL02_SELECT(Function)
...
-- 답 입학일을 형변환 함수 TO_CHAR를 이용해서 연도 4자리까지 표현
...
-- 2번
-- 춘 기술대학교의 교수 중 이름이 세 글자가 아닌 교수가 있다고 한다. 그 교수의 이름과 주민번호를 화면에 출력하는 SQL문장을 작성해보자.
-- (*이때 올바르게 작성한 SQL 문장의 결과 값이 예상과 다르게 나올 수 있다. 원인이 무엇인지 생각해볼 것) 
SELECT PROFESSOR_NAME, PROFESSOR_SSN 
FROM TB_PROFESSOR
WHERE LENGTH (PROFESSOR_NAME) != 3;

-- 답 LIKE로 '_' 와일드카드를 사용해서 정확히 3글자가 아닌 이름 검색
SELECT PROFESSOR_NAME, PROFESSOR_SSN
FROM TB_PROFESSOR
...
-- 답 SUBSTR으로 연도 2자리를 추출해 19와 연결해서 2000년도 전 출생년도로 바꿈 
-- 날짜 타입으로 연도 4자리로 형변환
-- 현재날짜와 형변환한 개월수 차이를 MONTHS_BETWEEN로 계산하고 12개월 연단위로 쪼갬
-- TRUNC 함수로 소수버림
...
ABS(
    TO_NUMBER(
        (TO_CHAR(TO_DATE(SUBSTR(STUDENT_SSN, 1,2),'YY'), 'YYYY')-100) -
        CASE 
           WHEN 
             TO_NUMBER(SUBSTR(ENTRANCE_DATE, 1, 2)) <= TO_NUMBER(TO_CHAR(SYSDATE, 'YY')) THEN '20' || SUBSTR(ENTRANCE_DATE, 1, 2)
           ELSE '19' || SUBSTR(ENTRANCE_DATE, 1, 2)
        END 
    )
)나이
FROM TB_STUDENT
...
FROM TB_STUDENT
WHERE 
    MONTHS_BETWEEN(ENTRANCE_DATE, TO_DATE(SUBSTR(STUDENT_SSN, 1, 6), 'RRMMDD'))/12 > 19
...
-- 이렇게 하면 학과가 나오지 않는 오류
SELECT DEPARTMENT_NO, COUNT(*)휴학생
FROM TB_STUDENT
WHERE ABSENCE_YN = 'Y'
GROUP BY DEPARTMENT_NO;

-- 답 DECODE를 사용해서 NULL처리,휴학생이아니면 NULL 
-- 그래서 휴학생이 없는 학과는 0으로 나오게해서 전체학과를 확인할 수 있게 함. 
...
SELECT SUBSTR(TERM_NO, 5, 6) "학기", ROUND(AVG(POINT), 1) "학기 별 평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
GROUP BY SUBSTR(TERM_NO, 5, 6);
