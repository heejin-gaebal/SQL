...
ALTER TABLE TB_CATEGORY 
    ADD CONSTRAINT CATEGORY_NAME PRIMARY KEY(CATEGORY_NAME);
...
ALTER TABLE TB_CATEGORY RENAME CONSTRAINT CATEGORY_NAME TO PK_NAME;
ALTER TABLE TB_CLASS_TYPE RENAME CONSTRAINT SYS_C008378 TO PK_CLASS_TYPE_NO;
...
ALTER TABLE TB_DEPARTMENT RENAME CONSTRAINT CATEGORY_NAME TO FK_TB_DEPARTMENT_CATEGORY;

-- 10번
-- 춘 기술대학교 학생들의 정보만이 포함되어 있는 학생일반정보 VIEW를 만들고자 한다.
-- 아래 내용을 참고하여 적절한 SQL문을 작성하시오.
CREATE VIEW 학생일반정보_VIEW
AS
SELECT STUDENT_NO, STUDENT_NAME, STUDENT_ADDRESS
FROM TB_STUDENT;

SELECT * FROM 학생일반정보_VIEW;
...
CREATE VIEW VW_STU_INFO(학생이름, 학과이름, 담당교수이름)
