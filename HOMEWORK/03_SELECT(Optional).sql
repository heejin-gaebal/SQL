...
SELECT STUDENT_NAME "학생 이름", STUDENT_SSN "주민번호"
FROM TB_STUDENT
WHERE ABSENCE_YN = 'Y'
...
FROM TB_STUDENT
WHERE SUBSTR(STUDENT_ADDRESS, 1,2) IN ('경기','강원') AND 
...
-- 학점이 높은 학생부터 표시하고,
-- 학점이 같으면 학번이 낮은 학생부터 표시하는 구문을 작성해 보시오. (학번자체 ASC)
-- 워크북 결과와 동일하게 소수점 아래 2자리까지 0으로 표현하기 위해서 TO_CHAR(NUMBER, 'FM9.00') 포맷 사용
SELECT TO_CHAR(POINT, 'FM9.00') 학점
FROM TB_GRADE
...
FROM TB_STUDENT S
LEFT JOIN TB_DEPARTMENT D ON S.DEPARTMENT_NO = D.DEPARTMENT_NAME
ORDER BY STUDENT_NAME ASC;

-- 7번
-- 춘 기술대학교의 과목 이름과 과목의 학과 이름을 출력하는 SQL문장을 작성하시오.
SELECT C.CLASS_NAME 과목_이름, D.DEPARTMENT_NAME 과목의_학과명
FROM TB_CLASS C
LEFT JOIN TB_DEPARTMENT D ON C.DEPARTMENT_NO = D.DEPARTMENT_NO;

-- 8번
-- 과목별 교수 이름을 찾으려고 한다. 과목 이름과 교수 이름을 출력하는 SQL문을 작성하시오.
SELECT DISTINCT C.CLASS_NAME "과목_이름", P.PROFESSOR_NAME
...
LEFT JOIN TB_CLASS C ON S.DEPARTMENT_NO = C.DEPARTMENT_NO
...
SELECT S.STUDENT_NAME "학생이름", NVL(COACH_PROFESSOR_NO, '지도교수 미지정') "지도교수"
FROM TB_STUDENT S
...
ORDER BY S.DEPARTMENT_NO ASC;

-- 15번
-- 휴학생이 아닌 학생 중 평점이 4.0 이상인 학생을 찾아 
-- 그 학생의 학번, 이름, 학과, 평점을 출력하는 SQL문을 작성하시오.
SELECT 
    S.STUDENT_NO 학번, S.STUDENT_NAME 이름, 
    D.DEPARTMENT_NAME 학과, ROUND(AVG(POINT), 1) 평점
FROM TB_STUDENT S
LEFT JOIN TB_DEPARTMENT D ON S.DEPARTMENT_NO = D.DEPARTMENT_NO
LEFT JOIN TB_GRADE G ON S.STUDENT_NO = G.STUDENT_NO
...
SELECT C.CLASS_NO, ROUND(AVG(POINT), 1)
FROM TB_DEPARTMENT D
...
SELECT S.STUDENT_NAME, MAX(POINT)
FROM TB_GRADE G
